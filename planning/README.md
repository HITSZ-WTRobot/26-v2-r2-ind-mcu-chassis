...
Curve 4 is generated by the standalone `../planning4/` planner. After verifying
it there, link or copy the resulting `../planning4/dist/trajectory_traj_4.hpp` into
`dist/` so the firmware can keep including all offline trajectories through the
existing `planning/dist` include path.
