...
def point_in_free_space_strict(x: float, y: float, eps: float = EPS_CLEARANCE) -> bool:
    in_union = (
        _inside_rect_open(x, y, ZONE1.x_min, ZONE1.x_max, ZONE1.y_min, ZONE1.y_max, eps)
        or _inside_rect_open(x, y, ZONE2.x_min, ZONE2.x_max, ZONE2.y_min, ZONE2.y_max, eps)
        or _inside_rect_open(x, y, ZONE3.x_min, ZONE3.x_max, ZONE3.y_min, ZONE3.y_max, eps)
    )
    in_thin = _inside_rect_closed(
        x,
        y,
        THIN_OBS_X_MIN,
        THIN_OBS_X_MAX,
    ...
    return in_union and not in_thin
