...
{
    auto it = std::find_if(kBaseCorridors.begin(), kBaseCorridors.end(), [&](const Corridor& c) {
        return name == c.name;
    });
...
{
    auto it = std::find_if(corridors.begin(), corridors.end(), [&](const Corridor& c) {
        return name == c.name;
    });
    if (it == corridors.end())
...
std::vector<NodeGuess> initial_state_nodes(int start_idx, const BranchSpec& branch)
{
    ...
    targets.push_back({EXIT_POINT.x, EXIT_POINT.y, EXIT_POINT.yaw_deg, EXIT_POINT.h});

    if (targets.size() != branch.phases.size()) {
        std::ostringstream oss;
        oss << "branch " << branch.suffix << " has " << branch.phases.size()
            << " phases but " << targets.size() << " phase targets";
        throw std::runtime_error(oss.str());
    }
...
std::vector<double> estimate_phase_durations(const std::vector<NodeGuess>& nodes, const BranchSpec& branch)
{
                                 ...
                                 double sample_dt)
{
    ...
    for (int i = 0; i < n_samples; ++i) {
        ...
        int idx = static_cast<int>(std::upper_bound(t_nodes.begin(), t_nodes.end(), t) - t_nodes.begin()) - 1;
...
BranchSolveResult solve_branch(int start_idx, const BranchSpec& branch, double sample_dt)
{
    ...
    try {
        ...
        auto result = sample_solution(branch.suffix, t_nodes, sol.value(X), sol.value(V), sample_dt);
