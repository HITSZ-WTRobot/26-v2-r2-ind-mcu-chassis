...
Curve 4 is generated by the standalone `planning4/` planner. After verifying it
there, link or copy the resulting `planning4/dist/trajectory_traj_4.hpp` into
`planning/dist/` so the firmware can keep including all offline trajectories
through the existing `planning/dist` include path.
...
The firmware-facing headers under `planning/dist` are:
...
- `dist/trajectory_all.hpp`: aggregate header generated by `planning/`; currently
...
verification, and plotting. Curve 4's inspection CSV is kept in
`planning4/dist/traj_4.csv`. Do not hand-edit files under `dist/`; change the
