...
| `0x21` | `LidarPosture` | `EnablePcLocalization` | 输入外部位姿观测；首个满足接入条件的包还负责触发延迟定位初始化 | `x*2000`、`y*2000`、`yaw*100`、`lidarTimestamp(ms)` |
...
- `LidarPosture`
  ...
  - 首个满足接入条件的外部位姿除了更新观测外，还负责写入 `System::Init::posture`
