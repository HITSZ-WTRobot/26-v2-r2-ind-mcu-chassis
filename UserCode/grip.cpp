...
Grip::Grip()
{
    ...
    const osThreadAttr_t grip_test_Task_attributes = { .name       = "grip_test_Task",
                                                       .stack_size = 256 * 8,
...
{
    // 电机堵转到限位处
    locked_set_initPosition(Gripping::parm_motor,
                            Gripping::parm_locked_vel,
                            (Grip::locked_config){ .locked_output        = 1000.0f,
                                                   .locked_deadAngle     = 0.1f,
                                                   .locked_speed         = -30.0f,
                                                   .locked_test_interval = 100 });
    locked_set_initPosition(Gripping::pturn_motor,
                            Gripping::pturn_locked_vel,
                            (Grip::locked_config){ .locked_output        = 500.0f,
                                                   .locked_deadAngle     = 0.1f,
                                                   .locked_speed         = -30.0f,
                                                   .locked_test_interval = 100 });
}
...
{
    HAL_GPIO_WritePin(GRIP_OUT_GPIO_Port,
                      GRIP_OUT_Pin,
                      static_cast<int>(grip_state) ? GPIO_PIN_SET : GPIO_PIN_RESET);
}

// 电机堵转到限位处
void Grip::locked_set_initPosition(motors::DJIMotor*                motor,
                                   ...
                                   const locked_config&             cfg)
{
    ...
    while (!is_locked)
    {
        if (std::abs(motor->getIqCMD()) > cfg.locked_output)
        {
            ...
            if (std::abs(motor->getAngle() - now_angle) < cfg.locked_deadAngle)
            { // 如果角度变化小于某个阈值，认为电机堵转
                is_locked = true;
                break;
            }
...
void motor_init()
{
    ...
    parm_vel_controller =
            new controllers::MotorVelController(parm_motor,
                                                Action::Grip_param::arm_vel_controller_cfg);
    pturn_vel_controller =
            new controllers::MotorVelController(pturn_motor,
                                                Action::Grip_param::turn_vel_controller_cfg);
    parm_trajectory = new trajectory::MotorTrajectory<1>(parm_vel_controller,
                                                         Action::Grip_param::arm_trajectory_cfg,
                                                         Action::Grip_param::arm_trajectory_pd_cfg);
    pturn_trajectory =
            new trajectory::MotorTrajectory<1>(pturn_vel_controller,
                                               Action::Grip_param::turn_trajectory_cfg,
                                               Action::Grip_param::turn_trajectory_pd_cfg);
