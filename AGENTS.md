...
- `UserCode/suction/` — reusable suction-cup component and module-local config: `Config.hpp` centralizes shared pressure-sensor parameters plus shared attach/release detection thresholds and fallback delays; `SuctionCup.*` implements the reusable cup behavior.
...
- Suction support is split into two compile-time layers: `ProjectParts::EnableGripSuction` controls whether the grip-side pump and KFS action flow exist at all, while `ProjectParts::EnableGripSuctionPressureSensor` only controls whether attach/release confirmation prefers the optional pressure sensor or falls back to delay-based confirmation.
...
- `Suction::SuctionCup` is a reusable component, not a grip-only type. Its public runtime interface should stay limited to suction on, suction off, and “has object”; pressure-sensor access remains an internal implementation detail. When `ProjectParts::EnableGripSuctionPressureSensor` is enabled and a fresh sample exists, `hasObject()` should prefer pressure-based Schmitt-trigger confirmation; otherwise it must fall back to delay-based attach/release confirmation. Keep only generic suction behavior and shared suction defaults inside `UserCode/suction/`; owner-specific wiring such as pump GPIO, I2C phase, and stale-time policy should live with the owning subsystem config.
- `Grip::Action::KfsStore` owns the only suction cup used by this project and a fixed no-parameter store/release flow. Its state machine should treat `Suction::SuctionCup::hasObject()` as the single source of truth for “picked” and “released”, rather than duplicating pressure polling or separate attach/release timers in the action layer. Keep its suction-pickup / temporary-store / release / standby poses and the owner-side suction config assembly centralized in `Grip::Config::KfsStore`, and update the config together with the action logic when that hardware sequence changes.
- `Protocol::ActionState::table` is a packed 16-bit feedback field. It currently encodes Step status, chassis mode + trajectory-finished flag, lift status, and grip/action status. The current chassis-mode subfield uses `Stop / Velocity / Position / Slave`, and the current grip subfield uses `Calibrating / TakingSpear / KfsStore / KfsRelease / Idle / Done`; if you change any subfield semantics, update `UserCode/protocol/ActionState.hpp`, the feedback protocol comment, and the upper-host agreement together.
