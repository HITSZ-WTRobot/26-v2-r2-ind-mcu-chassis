...
- `ProjectParts::EnableStepInfraredSwitchLogic` controls whether Step uses the red-infrared GPIO switch inputs for leg retract/deploy state transitions. When it is disabled, Step must use the original `currentRelativeX()` position-threshold logic instead; the switch feedback bits remain reported to the upper host either way.
...
- `Protocol::ActionState::table` is a packed 16-bit feedback field. It currently encodes Step status, chassis mode + trajectory-finished flag, lift status, grip/action status, a two-bit offline-trajectory state in bit10..11, and four red-infrared GPIO switch bits in bit12..15. The current chassis-mode subfield uses `Stop / Velocity / Position / Slave`, the current grip subfield uses `Calibrating / TakingSpear / KfsStore / KfsRelease / Idle / Done`, and the switch bits map directly to `Device::Switch::infrared_switch[0..3]` without Step direction remapping; if you change any subfield semantics, update `UserCode/protocol/ActionState.hpp`, the feedback protocol comment, and the upper-host agreement together.
